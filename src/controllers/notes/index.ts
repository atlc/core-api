...
import { RequestUser, Note } from "../../../lib/types";
import { notes } from "../../db/";
import { v4 as uuid_v4 } from "uuid";

const get_profile: RequestHandler = async (req: RequestUser, res, next) => {
    try {
...
const get_one: RequestHandler = async (req: RequestUser, res, next) => {
    try {
...
const create: RequestHandler = async (req: RequestUser, res, next) => {
    try {
        const id = uuid_v4();
        const user_id = req.user.id;
...
const change_pinned: RequestHandler = async (req: RequestUser, res, next) => {
    try {
...
const edit: RequestHandler = async (req: RequestUser, res, next) => {
    try {
        const note_id = req.params.id;
        const user_id = req.user.id;
        const { content }: Note = req.body;
...
const destroy: RequestHandler = async (req: RequestUser, res, next) => {
    try {
        const note_id = req.params.id;
        const user_id = req.user.id;
